...
namespace xpt {
    ...
    bool handleInputs(aPtr<acore::CommandInputs> &inputs, STLHelperInputs &hIn, STLHelperConfig &res,
                      STLHelperError *err = nullptr) {
        hIn = {
    ...
    bool generateFileList(STLHelperConfig const &conf, std::vector<STLHelperFileTaskInfo> &res,
                          STLHelperError *err = nullptr) {
    ...
    bool exportSTLFiles(std::vector<STLHelperFileTaskInfo> const &tasks, aPtr<aDesign> design,
                        STLHelperError *err = nullptr) {
	  // check Design
	  if (!design) {
		if (err) {
